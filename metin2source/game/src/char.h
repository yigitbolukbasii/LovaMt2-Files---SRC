...
//		bool			IsPet() const		{ return m_bCharType == CHAR_TYPE_PET; }
	...
	public:
		...
		{ 
			m_points.gold = gold;
			if(GetGold() < 0 || GetGold() > g_llMaxGold)
			{
				sys_err("[OVERFLOW_GOLD IN STEP 2] maximum %lld player %lld id %u Name %s ", g_llMaxGold, GetGold(), GetPlayerID(), GetName());
				m_points.gold = g_llMaxGold;
				return;
			}
		}
