...
public:
    ...
    void create(std::string key, const Token& t) {
        ...
        if (t.token == TK_STRING) {        
            ...
            error("Invalid define format. Use define CONSTANT \"value\"");
        }   
    }
...
struct AScript {
    ...
    AScript(string a, string b, string c) : when_condition(a),
    when_argument(b), script(c) {}
    AScript() {}
};
...
void parse(const char * filename) {
    ...
    if ((lf.f = fopen(filename, "r")) == NULL)
    {
        error("Can't open the file. Make sure it exists.");
    }
        ...
        switch (ps) {
            ...
            case ST_WHEN_NAME:
            {
                ...
                if (t.token == TK_NAME || t.token == TK_STRING || t.token == TK_NUMBER) {
                    if (t.token == TK_NUMBER) {
                        ...
                        if (constant.exists(current_when_name)) {
                    ...
                    if (!current_when_argument.empty()) {
                        ...
                        if (fpos != string::npos) {
                            ...
                            if (fpos < current_when_argument.size() && current_when_argument[fpos] == '.') {
                                ...
                                error("tra function it's used incorrectly");
                            }
                        }
            ...
            case ST_FUNCTION_ARG:
            {
                ...
                if (t.token != ')') {
                    do {
                        if (t.token == TK_NAME) {
                            current_function_arg += token2string(t);
                            next(&lexstate);
                            if (t.token != ')')
                                current_function_arg += ',';
                        } else {
                            ostringstream os;
                            os << "invalud argument name " << token2string(t) << " for function " << current_function_name;
                            error(os.str().c_str());
                        }
