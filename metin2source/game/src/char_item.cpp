	...
	case INVENTORY:
		{
			...
			else
			{
				...
				do
				{
					...
					if (bigger_inventory == true && (p / (inventorysize / 3) != bPage))
						return false;
...
void CHARACTER::__OpenPrivateShop()
{
	...
	switch (bodyPart)
	{
		...
		case 2:
			...
			{
				ChatPacket(CHAT_TYPE_INFO, "GameMaster pazar kuramaz.");
			}
			else
			{
					...
					case USE_ADD_ATTRIBUTE2:
						{
							...
							switch (item->GetSubType())
							{
								...
								case USE_CHANGE_ATTRIBUTE :
									...
									if (true == item->isLocked())
									{
										ChatPacket(CHAT_TYPE_INFO, "Bu iteme efsun atamazsın.");
										return false;
									}
...
bool CHARACTER::DropItem(TItemPos Cell, BYTE bCount)
{
	...
	if (GetGMLevel() > GM_PLAYER && GetGMLevel() < GM_IMPLEMENTOR)
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("GameMaster yere item atamaz."));
		return false;
	}
