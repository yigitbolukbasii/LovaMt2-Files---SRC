...
int CInputMain::Chat(LPCHARACTER ch, const char * data, size_t uiBytes)
{
	...
	if (pinfo->type == CHAT_TYPE_SHOUT)
	{
		...
		if (thecore_heart->pulse - (int) ch->GetLastShoutPulse() < passes_per_sec * 10)
			return (iExtraLen);
...
void CInputMain::SafeboxCheckin(LPCHARACTER ch, const char * c_pData)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "GameMaster depo kullanamaz.");
		return;
	}
...
void CInputMain::AnswerMakeGuild(LPCHARACTER ch, const char* c_pData)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "<Lonca> 200.000 yang gerekli.");
		return;
	}
	if (ch->GetLevel() < 40)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "<Lonca> 40. seviye olman gerekir. ");
		return;
	}
